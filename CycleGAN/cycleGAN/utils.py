...
import torch
from opt.data_options import Data_Options
...
def create_cyclegan2():
    opt = Data_Options().parse()
    if opt.model == 'whistle_cycle_deepContour_negID_residual':
        from gan_models.whistle_cycleGAN_deepcontour_negId_model_residual import WhistleCycleGANDeepContourNegIDModel_Res
        model = WhistleCycleGANDeepContourNegIDModel_Res()
    else:
        raise NotImplementedError('model [%s] not implemented.' % type)
...
def create_cyclegan3():
    opt = Data_Options().parse()
    opt.netG_A_residual = False
    opt.netG_A_rlow = 0
    opt.netG_A_rhigh = 0
    opt.isTrain = False
    if opt.model == 'whistle_cycle_deepContour_negID_residual':
...
def generate_fakeB_residual(model, pos, neg):
    inputA = torch.cat([pos, neg], dim=1)
    model.input_A = inputA
    with torch.no_grad():
        model.test()
    model.fake_B_res.detach()
    return model.fake_B_res