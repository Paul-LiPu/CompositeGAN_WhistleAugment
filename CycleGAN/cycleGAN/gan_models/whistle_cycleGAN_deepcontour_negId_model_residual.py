...
class WhistleCycleGANDeepContourNegIDModel_Res(BaseModel):
    ...
    def initialize(self, opt):
        ...
        with torch.no_grad():
            self.zero = torch.zeros(self.opt.batchSize, 1, self.opt.loadSize, self.opt.loadSize).cuda()
        self.opt = opt
    ...
    def backward_G(self):
        ...
        # self.fake_B_neg = self.netG_A(self.real_A_neg)
        ...
        # self.real_B_neg_all = self.real_B_neg.clone()
        # data_size = self.real_B_neg_all.size()
        # self.real_B_neg_all = self.real_B_neg_all.expand((data_size[0], 2, data_size[2], data_size[3]))
        # print self.real_B_neg_all.size()
        # print self.fake_A_neg.size()
        # self.loss_fake_A_neg_Idt = self.criterionIdt(self.fake_A_neg[:, 1:, :, :], self.real_B_neg) + self.criterionIdt(self.fake_A_neg[:, :1, :, :], self.zero)

        self.zero = Variable(torch.zeros(self.real_B_neg.size(0), 1, self.opt.loadSize, self.opt.loadSize),
                             volatile=True).cuda()
        self.loss_fake_A_neg_Idt = self.criterionIdt(self.fake_A_neg[:, 1:, :, :], self.real_B_neg) + self.criterionIdt(self.fake_A_neg[:, :1, :, :], self.zero)
